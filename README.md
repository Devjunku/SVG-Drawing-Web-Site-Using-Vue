...
위와 같이 대표적인 8개의 태그 중 `<rect>`, `<circle>`, `<line>`, `<polyline>`, `<text>`, `<image>`를 사용하여 기본적인 도형과 선을 생성할 수 있도록 하겠습니다.
...
모든 태그는 기본적으로 사용방법이 모두 동일합니다. event에 기록된 clientX, clientY를 접근하여 해당 좌표시작으로 그리기 시작합니다.
...
6. import 되지 않는 이슈를 확인했습니다.

   - 오탈자 수정 후 완료되었습니다.

7. export 되지 않는 이슈를 확인했습니다.

   - 오탈자 수정 후 완료되었습니다.

8. image(jpeg, png etc)를 불러올 수 없던 이슈를 확인했습니다.
